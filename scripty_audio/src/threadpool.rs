...
static THREADPOOL_SUBMIT: OnceCell<mpsc::Sender<Box<dyn FnOnce()>>> = OnceCell::new();
...
pub fn submit_job<T>(f: Box<dyn FnOnce(oneshot::Sender<T>) -> ()>) -> Result<T, TokioRecvError> {
...
pub async fn submit_job_async<T>(
    f: Box<dyn FnOnce(oneshot::Sender<T>) -> ()>,
