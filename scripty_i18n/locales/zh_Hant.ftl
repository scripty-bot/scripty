...
donation-description =
    ...
    訓練模型同樣不容易, 因為這需要相對較新的 (支持 CUDA 10.1) 英偉達顯卡
    我們不喜歡問別人捐款，但我們沒有能力自掏腰包運行 Scripty 因為它運行起來太貴了
    所以我們正在尋求幫助，並以訂閱的形式給予獎勵\n\n
    您可以在網站 https://scripty.org/premium 查看更多信息，但主要的要點是，
    價格從每月 5 美元到 100 美元不等.
    100 美元的套餐包括單獨為你的伺服器服務的 Scripty，自定義名稱和頭像。\n\n
...
root-command-invoked-description =
    請僅使用此指令的子指令。\n
    請參閱`{ $contextPrefix }help { $commandName }` 以獲取更多信息。
...
no-channel-specified =
    您不在語音頻道中，也沒有指定要加入的頻道。
    用`{ $contextPrefix }join <channel>` 指定語音頻道，或自己進入語音頻道後重新使用此指令。
...
language-set-failure-description-db =
    數據庫在嘗試設置你的語言時遇到錯誤。
    已報告此錯誤，我們將對其進行調查。請不要重複使用該指令。 (假如你好奇的話, 以下是該錯誤。{ $error })
...
webhook-deleted =
    看起來你刪除了我使用的 webhook！ *打*
    重新運行 `{ $contextPrefix }setup` 以解決這個問題。
