...
command-not-found = Imperatus cum nomen `{ $commandName }` non est.
...
guild-language-set-success = Iam lingua communitatem `{ $language }` est.
guild-language-set-success-description = Ut ad Anglicum revertas, dice `{ $contextPrefix }language user_language en`.
language-set-failure-title-unsupported = Lingua tu velle in programmatem non est.
